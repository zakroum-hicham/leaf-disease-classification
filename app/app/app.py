...
def potato():
    ...
    prediction = get_model("potato").predict(image)
    ...
    if image_file:
            ...
            filepath = os.path.join(".\\app\\app\\static\\uploads", name)
...
def pepper():
    ...
    # pass the data too the model
    prediction = get_model("pepper").predict(image)
    predicted_class = get_classes("pepper")[np.argmax(prediction[0])]
    confidence = np.max(prediction[0])
    # Return the predicted class and the confidence value
    if image_file:
            filename = secure_filename(image_file.filename)
...
def tomato():
    ...
    # pass the data too the model
    prediction = get_model("tomato").predict(image)
    predicted_class = get_classes("tomato")[np.argmax(prediction[0])]
    confidence = np.max(prediction[0])
    # Return the predicted class and the confidence value
    if image_file:
            filename = secure_filename(image_file.filename)
